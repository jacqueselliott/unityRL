...
class Qnetwork():
    def __init__(self):
        ...
        self.fc_1 = tf.contrib.layers.fully_connected(self.state, 300, activation_fn=tf.nn.relu)
        self.fc_2 = tf.contrib.layers.fully_connected(self.fc_1, 300, activation_fn=tf.nn.relu)
        self.fc_3 = tf.contrib.layers.fully_connected(self.fc_2, 300, activation_fn=tf.nn.relu)
        self.fc_4 = tf.contrib.layers.fully_connected(self.fc_3, 200, activation_fn=tf.nn.relu)
        self.fc_5 = tf.contrib.layers.fully_connected(self.fc_4, 100, activation_fn=tf.nn.relu)
        self.fc_6 = tf.contrib.layers.fully_connected(self.fc_5, 100, activation_fn=tf.nn.relu)
        self.q_output = tf.contrib.layers.fully_connected(self.fc_6, self.num_actions, activation_fn=None)
        # # HIDDEN LAYER ONE
...
def main(ws):
    ...
    max_epLength = 800 #The max allowed length of our episode.
    load_model = True #Whether to load a saved model.
    path = "./dqn_windows_new" #The path to save our model to.
            ...
            while j < max_epLength: #If the agent takes longer than 1000 moves to finish, end the trial.
                ...
                if total_steps > pre_train_steps:
                    ...
                    if total_steps % (update_freq) == 0:
                        ...
                        actions_vector = trainBatch[:,1].astype(int)
                        b_size = len(actions_vector)
                        temp_onehot = np.zeros((b_size,4))
                        temp_onehot[np.arange(b_size), actions_vector] = 1
                        _ = sess.run(mainQN.updateModel, \
                            feed_dict={mainQN.state:np.vstack(trainBatch[:,0]),mainQN.targetQ:targetQ, mainQN.actions_onehot:temp_onehot})
...
def calc_reward(drone_pos, target_pos):
    ...
    # reward = 0
    # reward = (0 - 0.5/(3*math.sqrt(2))*dist)
    # if reward > 0:
    #     reward = 0
    # if reward < -1.5:
    #     reward = -1.5
...
# hardcoding this for velocity included in update
